        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                <div class="mb-6">
                    ...
                    <p class="text-sm text-slate-500 mt-1">Your assigned trips for {{ now()->format('d M, Y') }}</p>
                </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-emerald-600 mt-0.5">
                                    {{ $activeAssignment->scheduleDate->schedule->route->origin->name }} → {{ $activeAssignment->scheduleDate->schedule->route->destination->name }}
                                </p>
                            </div>
                        </div>
                        <a href="{{ route('driver.trip', $activeAssignment) }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-md transition-colors shrink-0">
                            Go to Trip
                        </a>
                    </div>
                </div>
                    ...
                    <div class="bg-white border border-slate-100 rounded-md shadow-sm p-5">
                        <div class="flex items-center justify-between mb-4">
                            <span class="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-md
                                @if($assignment->status == 'active') bg-emerald-50 text-emerald-700
                                @elseif($assignment->status == 'completed') bg-primary/10 text-primary
                                @else bg-amber-50 text-amber-700
                                @endif">
                                {{ ucfirst($assignment->status) }}
                            </span>
                            <span class="text-xs text-slate-400">{{ $assignment->scheduleDate->departure_date->format('d M Y') }}</span>
                        </div>

                        <h3 class="text-base font-semibold text-slate-900 mb-3">
                            {{ $assignment->scheduleDate->schedule->route->origin->name }} → {{ $assignment->scheduleDate->schedule->route->destination->name }}
                        </h3>
                        ...
                        <div class="space-y-1.5 text-sm text-slate-600">
                            <div class="flex items-center gap-2">
                                ...
                                <span>{{ $assignment->scheduleDate->schedule->bus->registration_number }} {{ $assignment->scheduleDate->schedule->bus->name ? '- '.$assignment->scheduleDate->schedule->bus->name : '' }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span>{{ \Carbon\Carbon::parse($assignment->scheduleDate->schedule->departure_time)->format('h:i A') }} — {{ \Carbon\Carbon::parse($assignment->scheduleDate->schedule->arrival_time)->format('h:i A') }}</span>
                            </div>
                        </div>

                        <div class="mt-5">
                            @if($assignment->status === 'assigned')
                            <form action="{{ route('driver.trip.start', $assignment) }}" method="POST">
                                @csrf
                            ...
                            <a href="{{ route('driver.trip', $assignment) }}" class="block w-full text-center px-4 py-2 bg-primary hover:bg-primary-dark text-white text-sm font-medium rounded-md transition-colors">
                                View Active Trip
                            </a>
                            @else
