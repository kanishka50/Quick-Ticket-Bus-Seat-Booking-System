        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('provider.assignments.create') }}" class="inline-flex items-center px-4 py-2 bg-primary hover:bg-primary-dark text-white text-sm font-medium rounded-md transition-colors">
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-slate-50">
                                ...
                                <tr class="hover:bg-slate-50/50">
                                    <td class="px-5 py-3">
                                        <span class="text-sm font-medium text-slate-900">{{ $assignment->driver->user->name }}</span>
                                        <span class="block text-xs text-slate-500">{{ $assignment->driver->phone ?? '-' }}</span>
                                    </td>
                                    ...
                                    <td class="px-5 py-3 text-sm text-slate-600">
                                        {{ $assignment->scheduleDate->schedule->bus->registration_number }}
                                    </td>
                                    <td class="px-5 py-3 text-sm text-slate-900">
                                        {{ $assignment->scheduleDate->departure_date->format('d M Y') }}
                                    </td>
                                    <td class="px-5 py-3 text-sm text-slate-600">
                                        {{ \Carbon\Carbon::parse($assignment->scheduleDate->schedule->departure_time)->format('h:i A') }} - {{ \Carbon\Carbon::parse($assignment->scheduleDate->schedule->arrival_time)->format('h:i A') }}
                                    </td>
                                    <td class="px-5 py-3">
                                    ...
                                    <td class="px-5 py-3 text-right">
                                        ...
                                        <form action="{{ route('provider.assignments.destroy', $assignment) }}" method="POST" class="inline" onsubmit="return confirm('Remove this assignment?')">
                                            @csrf
                                            @method('DELETE')
