        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="bg-white border border-slate-100 rounded-md shadow-sm p-6">
                    <form action="{{ route('provider.schedules.update', $schedule) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <input type="time" name="departure_time" id="departure_time" value="{{ old('departure_time', $schedule->departure_time) }}"
                            ...
                            <div>
                                ...
                                <input type="time" name="arrival_time" id="arrival_time" value="{{ old('arrival_time', $schedule->arrival_time) }}"
                                    class="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" required>
                                @error('arrival_time')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="price" id="price" min="0.01" step="0.01" value="{{ old('price', $schedule->price) }}"
                ...
                <div class="bg-white border border-slate-100 rounded-md shadow-sm p-6 mt-6">
                    <div class="flex items-center mb-4">
                        ...
                        <h2 class="text-sm font-semibold text-amber-700">Active Bookings on Future Dates ({{ $futureBookings->count() }})</h2>
                    </div>
                    <div class="border border-amber-200 bg-amber-50 rounded-md p-4 mb-4">
                        <div class="flex items-center justify-between">
                            ...
                            <form action="{{ route('provider.schedules.request-cancellation', $schedule) }}" method="POST" class="ml-4 flex-shrink-0">
                                @csrf
                                <button type="submit" class="px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-md hover:bg-red-700 transition-colors" onclick="return confirm('This will send a cancellation request to admin for all {{ $futureBookings->count() }} booking(s). Continue?')">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-slate-50">
                                ...
                                <tr>
                                    <td class="px-4 py-2.5 text-sm text-slate-900">{{ $booking->scheduleDate->departure_date->format('d M Y') }}</td>
                                    <td class="px-4 py-2.5 text-sm text-slate-900">{{ $booking->booking_number }}</td>
                                    ...
                                    <td class="px-4 py-2.5 text-sm text-slate-600">{{ $booking->seatBookings->pluck('seat_number')->join(', ') }}</td>
                                    <td class="px-4 py-2.5 text-sm text-slate-900">LKR {{ number_format($booking->total_amount, 2) }}</td>
                                    <td class="px-4 py-2.5">
