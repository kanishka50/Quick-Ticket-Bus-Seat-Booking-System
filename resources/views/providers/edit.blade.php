        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="bg-white border border-slate-100 rounded-md shadow-sm p-6">
                    <form method="POST" action="{{ route('provider.update') }}" enctype="multipart/form-data" class="space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input id="company_name" type="text" name="company_name" value="{{ old('company_name', $provider->company_name) }}" required
                        ...
                        <div>
                            ...
                            <input id="business_registration_number" type="text" name="business_registration_number" value="{{ old('business_registration_number', $provider->business_registration_number) }}" required
                                class="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
                        </div>
                        ...
                        <div>
                            ...
                            <input id="contact_number" type="text" name="contact_number" value="{{ old('contact_number', $provider->contact_number) }}" required
                                class="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="address" name="address" rows="2" required
                                class="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">{{ old('address', $provider->address) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                class="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">{{ old('description', $provider->description) }}</textarea>
                        </div>
                            ...
                            <div class="flex items-center gap-4">
                                <div class="relative">
                                    <div class="w-16 h-16 rounded-md overflow-hidden bg-slate-100 border border-slate-200">
                                        <img id="logo-preview"
                                             src="{{ $provider->company_logo ? asset('storage/' . $provider->company_logo) : '' }}"
                                             alt="Company Logo"
                                             class="w-full h-full object-cover {{ $provider->company_logo ? '' : 'hidden' }}">
                                        <div id="logo-placeholder" class="w-full h-full flex items-center justify-center {{ $provider->company_logo ? 'hidden' : '' }}">
