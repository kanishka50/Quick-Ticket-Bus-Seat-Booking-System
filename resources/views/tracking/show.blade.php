...
<div class="bg-surface min-h-screen">
    <div class="container mx-auto px-4 max-w-5xl py-8">
        ...
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('bookings.show', $booking) }}" class="w-9 h-9 bg-white border border-slate-200 rounded-md flex items-center justify-center text-slate-500 hover:text-primary hover:border-primary transition-colors">
        ...
        <div class="bg-white border border-slate-100 rounded-md shadow-sm p-4 mb-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <p class="text-sm font-semibold text-slate-900 mt-0.5">#{{ $booking->booking_number }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-sm font-semibold text-slate-900 mt-0.5">{{ $booking->scheduleDate->departure_date->format('d M Y') }}</p>
                </div>
                <div>
                    <p class="text-xs text-slate-500 uppercase tracking-wide">Bus</p>
                    <p class="text-sm font-semibold text-slate-900 mt-0.5">{{ $booking->scheduleDate->schedule->bus->registration_number }}</p>
                </div>
            </div>
        </div>

        @if($booking->scheduleDate->driverAssignment && $booking->scheduleDate->driverAssignment->status === 'active')
            <!-- Live Tracking -->
            <div class="bg-white border border-slate-100 rounded-md shadow-sm mb-6">
                ...
                <div class="px-5 py-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <p id="bus-location" class="text-sm font-medium text-slate-900 mt-0.5">{{ $location ? ($location->location_name ? 'Near ' . $location->location_name : 'Lat: ' . $location->latitude . ', Lng: ' . $location->longitude) : 'Waiting for update...' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 uppercase tracking-wide">Speed</p>
                        <p id="bus-speed" class="text-sm font-medium text-slate-900 mt-0.5">{{ $location && $location->speed ? $location->speed . ' km/h' : '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 uppercase tracking-wide">Last Updated</p>
                        <p id="last-updated" class="text-sm font-medium text-slate-900 mt-0.5">{{ $location ? $location->recorded_at->diffForHumans() : '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                const scheduleDateId = {{ $booking->schedule_date_id }};

                let lat = {{ $location ? $location->latitude : 7.8731 }};
                let lng = {{ $location ? $location->longitude : 80.7718 }};
                let hasLocation = {{ $location ? 'true' : 'false' }};
        ...
        @elseif($booking->scheduleDate->driverAssignment)
            ...
            <div class="bg-white border border-slate-100 rounded-md shadow-sm p-12 text-center">
                ...
                <p class="text-xs text-slate-400 mt-3">
                    <i class="fas fa-user mr-1"></i> Driver: {{ $booking->scheduleDate->driverAssignment->driver->user->name }}
                </p>
            </div>
        @else
            <!-- No Driver Assigned -->
