        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="bg-white border border-slate-100 rounded-md shadow-sm mb-6">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="p-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <textarea id="address" name="address" rows="1"
                                    class="w-full px-3 py-2 border border-slate-200 rounded-md text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors">{{ auth()->user()->address }}</textarea>
                                @error('address')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
